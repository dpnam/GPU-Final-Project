...
__global__ void inBlockSort(uint32_t *in, int n, int nBins, int bit,
  ...
  for (int b = 0; b < nBits; ++b) {
    ...
    for (int stride = 1; stride < 2 * blockDim.x; stride *= 2) {
      int s_inBinScanIdx =
          (threadIdx.x + 1) * 2 * stride - 1; // To avoid warp divergence
      if (s_inBinScanIdx < 2 * blockDim.x)
        s_inBinScan[s_inBinScanIdx] += s_inBinScan[s_inBinScanIdx - stride];
      __syncthreads();
    }

    // 2. Post-reduction phase
    for (int stride = blockDim.x / 2; stride > 0; stride /= 2) {
      int s_inBinScanIdx = (threadIdx.x + 1) * 2 * stride - 1 + stride; // Wow
      if (s_inBinScanIdx < 2 * blockDim.x)
        s_inBinScan[s_inBinScanIdx] += s_inBinScan[s_inBinScanIdx - stride];
      __syncthreads();
    }
...
void sortByDevice(const uint32_t *in, int n, uint32_t *out, int nBits,
  ...
  const dim3 sortBlockSize = dim3(histBlockSize.x / 2);
  const dim3 sortGridSize = dim3((n - 1) / (2 * sortBlockSize.x) + 1);

  const size_t SORT_SMEM_BYTES = 2 * 5 * sortBlockSize.x * sizeof(uint32_t);

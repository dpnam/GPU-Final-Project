...
__global__ void inBlockSort(uint32_t *in, int n, int nBins, int bit,
  ...
  uint32_t *s_temp = (uint32_t *)(s_outBin + s_n);
  ...
  for (int b = 0; b < nBits; ++b) {
    ...
    s_inBinScan[2 * threadIdx.x] = (s_inBin[2 * threadIdx.x] >> b) & 1;
    s_inBinScan[2 * threadIdx.x + 1] = (s_inBin[2 * threadIdx.x + 1] >> b) & 1;

    __syncthreads();

    int ai = threadIdx.x;
    int bi = threadIdx.x + (s_n / 2);
    int bankOffsetA = CONFLICT_FREE_OFFSET(ai);
    int bankOffsetB = CONFLICT_FREE_OFFSET(bi);
    s_temp[ai + bankOffsetA] = s_inBinScan[ai];
    s_temp[bi + bankOffsetB] = s_inBinScan[bi];

    int offset = 1;

    for (int stride = s_n >> 1; stride > 0; stride >>= 1) {
      __syncthreads();
    ...
    if (threadIdx.x == 0) {
      s_temp[s_n - 1 + CONFLICT_FREE_OFFSET(s_n - 1)] = 0;
    }

    for (int stride = 1; stride < s_n; stride *= 2) {
      offset >>= 1;
      __syncthreads();
    ...
    s_inBinScan[ai] = s_temp[ai + bankOffsetA];
    s_inBinScan[bi] = s_temp[bi + bankOffsetB];

    __syncthreads();

    /*
    s_inBinScan[threadIdx.x] = (threadIdx.x == 0) ? 0 : (s_inBin[threadIdx.x -
    1] >> b) & 1; s_inBinScan[threadIdx.x + blockDim.x] = (s_inBin[threadIdx.x +
    blockDim.x - 1] >> b) & 1;
    __syncthreads();
        ...
        {
                int s_inBinScanIdx = (threadIdx.x + 1) * 2 * stride - 1; // To
    avoid warp divergence if (s_inBinScanIdx < 2 * blockDim.x)
                        s_inBinScan[s_inBinScanIdx] +=
    s_inBinScan[s_inBinScanIdx - stride];
                __syncthreads();
        }
        ...
        {
                int s_inBinScanIdx = (threadIdx.x + 1) * 2 * stride - 1 +
    stride; // Wow if (s_inBinScanIdx < 2 * blockDim.x)
                        s_inBinScan[s_inBinScanIdx] +=
    s_inBinScan[s_inBinScanIdx - stride];
                __syncthreads();
        }
  ...
  if (i1 < n) {
    in[i1] = s_in[threadIdx.x];
  }
  if (i2 < n) {
    in[i2] = s_in[threadIdx.x + blockDim.x];
  }
}
...
void sortByDevice(const uint32_t *in, int n, uint32_t *out, int nBits,
  ...
  const dim3 sortBlockSize = dim3(blockSizes[0] / 2);
  ...
  for (int bit = 0; bit < 8 * sizeof(uint32_t); bit += nBits) {
    //// calculate local histogram of each block, transpose and copy to d_hist
    timer.Start();
    computeHistKernel<<<histGridSize, histBlockSize, HIST_SMEM_BYTES>>>(
        d_in, n, d_hist, nBins, bit);
    CHECK(cudaGetLastError());
    timer.Stop();
    printf("Local histogram: %.3f ms\n", timer.Elapsed());
    ...
    /* timer.Start(); */
    /* bool sorted; */
    /* isSorted<<<histGridSize, histBlockSize>>>(d_in, n, d_sorted); */
    /* CHECK(cudaGetLastError()); */
    /* CHECK(cudaMemcpy(&sorted, d_sorted, sizeof(bool),

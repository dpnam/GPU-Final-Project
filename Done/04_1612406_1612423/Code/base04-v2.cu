...
__global__ void scatter(const uint32_t *in, int n, const uint32_t *histScan,
                        uint32_t *out, int nBins, int bit) {
  const int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= n) {
    return;
  }

  const int bin = (in[i] >> bit) & (nBins - 1);
  int rank = bin * (gridDim.x - 1);
  out[rank] = in[i];
}
...
void sortByDevice(const uint32_t *in, int n, uint32_t *out, int nBits,
  ...
  /* printf("Hist block count: %d, scan block count: %d\n", histBlockCount, */
  /*        scanBlockCount); */
  /* printf("nBins: %d\n", nBins); */
  ...
  uint32_t *histScan = (uint32_t *)malloc(HIST_BYTES);
  uint32_t *src = (uint32_t *)malloc(ARRAY_BYTES);
  ...
  for (int bit = 0; bit < 8 * sizeof(uint32_t); bit += nBits) {
    ...
    /* printf("d_hist:\n"); */
    /* printDeviceArray(d_hist, nBins * histBlockCount); */
    ...
    CHECK(cudaGetLastError());
    /* printf("d_histScan before sum:\n"); */
    /* printDeviceArray(d_histScan, histBlockCount * nBins); */
    ...
    /* printf("d_blkSums:\n"); */
    /* printDeviceArray(d_blkSums, histBlockCount); */
    ...
    CHECK(cudaGetLastError());
    /* printf("d_histScan after sum:\n"); */
    /* printDeviceArray(d_histScan, nBins * histBlockCount); */
    ...
    /* scatter<<<histGridSize, histBlockSize>>>(d_in, n, d_histScan, d_out, nBins, */
    /*                                          bit); */

    // TODO: From "histScan", scatter elements in "src" to correct locations in
    // "dst"

    cudaMemcpy(histScan, d_histScan, HIST_BYTES, cudaMemcpyDeviceToHost);
    cudaMemcpy(src, d_in, ARRAY_BYTES, cudaMemcpyDeviceToHost);
    for (int i = 0; i < n; ++i) {
      int bin = (src[i] >> bit) & (nBins - 1);
      /* int scanIdx = histBlockCount * bin + histBlockCount - 1; */
      int scanIdx = histBlockCount * bin;
      out[histScan[scanIdx]++] = src[i];
    }
    cudaMemcpy(d_out, out, ARRAY_BYTES, cudaMemcpyHostToDevice);

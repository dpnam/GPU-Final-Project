...
__global__ void scatter(uint32_t *in, int n, const uint32_t *histScan,
  ...
  const uint32_t s_n = 2 * blockDim.x;
  ...
  for (int b = 0; b < nBits; ++b) {
    ...
    // 2nd
    uint32_t scanVal2 = (s_inBin[tid + blockDim.x - 1] >> b) & 1;
    // scan each warp
    for (int stride = 1; stride < 32; stride *= 2) {
      uint32_t strideVal = __shfl_up_sync(0xffffffff, scanVal2, stride);
      if (tid % 32 >= stride)
        scanVal2 += strideVal;
    }
    // store warp sum
    if (tid % 32 == 31)
      s_warpSum[tid / 32] = scanVal2;

    scanVal2 += *s_lastScanVal;
    ...
    // add sum of previous warps
    if (tid >= 32)
      scanVal2 += s_warpSum[tid / 32 - 1];

    if (tid == blockDim.x - 1) {
      *s_lastScanVal = scanVal2;
    }
    ...
    if ((s_inBin[tid + blockDim.x] >> b) & 1) {
      const uint32_t nZeros =
          s_n - *s_lastScanVal - ((s_inBin[s_n - 1] >> b) & 1);
      rank = nZeros + scanVal2;
    } else {
      rank = tid + blockDim.x - scanVal2;
    }
    s_outBin[rank] = s_inBin[tid + blockDim.x];
    s_out[rank] = s_in[tid + blockDim.x];
...
void sortByDevice(const uint32_t *in, int n, uint32_t *out, int nBits,
  ...
  const size_t SCATTER_SMEM_BYTES =
      (2 * 4 * scatterBlockSize.x + nBins + 33) * sizeof(uint32_t);
